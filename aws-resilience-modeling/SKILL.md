...
- 详见 [resilience-framework.md](references/resilience-framework.md)
...
完整缓解策略示例参见 [example-report-template.md](assets/example-report-template.md)。
...
**3. 事后复盘流程（Postmortem）**：遵循无责任文化原则，使用标准复盘模板（时间线、根因、影响、行动项）。复盘模板示例参见 [example-report-template.md](assets/example-report-template.md)。
...
> 该数据格式遵循 [assessment-output-spec.md](references/assessment-output-spec.md) 规范，供下游 `chaos-engineering-on-aws` skill 直接消费。

**输出方式**（两种，默认推荐方式 1）：
1. **嵌入模式（推荐）**：在评估报告末尾添加 `## Chaos Engineering Ready Data` 附录
2. **独立文件模式**：单独生成 `{project}-chaos-input-{date}.md`

**必须包含的 8 个结构化章节**（表头和字段名固定，详见 [assessment-output-spec.md](references/assessment-output-spec.md)）：
...
**输出检查清单和可实验性判断指南**详见 [assessment-output-spec.md](references/assessment-output-spec.md)。
...
详细的报告生成流程、Python 模板代码、质量检查清单和工具安装检查参见 [report-generation.md](references/report-generation.md)。
HTML 模板使用说明参见 [HTML-TEMPLATE-USAGE.md](references/HTML-TEMPLATE-USAGE.md)。
