...
**验证点**：ASG 自动启动替代实例，ALB 健康检查通过，请求无中断
...
|------|------|------|
| 注入 | T+0s | 目标实例被终止 |
| 检测 | T+10-30s | ALB 健康检查检测到实例不可用 |
| ASG 响应 | T+30-60s | ASG 启动新实例 |
| 恢复 | T+180-300s | 新实例通过健康检查，流量恢复 |

**如果失败**：说明 ASG 配置有问题（min capacity、健康检查间隔、启动模板），需检查 ASG 策略和 Launch Template。
