...
**架构模式**：Aurora 集群（Writer + Reader）
**FIS Action**：`aws:rds:failover-db-cluster`
**验证点**：Reader 提升为 Writer，应用连接自动恢复，数据零丢失
...
| 恢复 | T+20-35s | 应用连接池重连，写入恢复 |

**如果失败**：常见原因——应用未使用 Aurora 集群端点（用了实例端点）、连接池无重试逻辑、DNS TTL 过长。检查连接字符串和重试配置。
