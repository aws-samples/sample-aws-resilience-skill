#!/usr/bin/env python3
"""Render a FERPA Markdown assessment report to a single self-contained HTML file.

Usage:
    python3 generate-html-report.py ferpa-reports/ferpa-assessment-2026-04-29.md

Produces a sibling .html file. Colors severity badges, scorecards, and status rows.
No third-party deps — uses a tiny Markdown converter (markdown2 if available,
else a minimal built-in fallback good enough for the fixed template).
"""
...
:root {
  ...
  --breach: #ff5576; --compliance: #ff9a4d; --hardening: #ffd166; --info: #4ade80;
...
.sev-breach     { background: rgba(255,85,118,0.15); color: var(--breach);     border: 1px solid var(--breach); }
.sev-compliance { background: rgba(255,154,77,0.15); color: var(--compliance); border: 1px solid var(--compliance); }
.sev-hardening  { background: rgba(255,209,102,0.15); color: var(--hardening); border: 1px solid var(--hardening); }
...
.disclaimer { margin-top: 12px; font-style: italic; color: var(--muted); }
"""
...
# FERPA-specific severity labels
SEV_MAP = [
    ("BREACH RISK",     "sev-breach"),
    ("COMPLIANCE GAP",  "sev-compliance"),
    ("HARDENING GAP",   "sev-hardening"),
    ("INFO",            "sev-info"),
]
STATUS_MAP = [
...
FOOTER_DISCLAIMER = (
    "This report is a point-in-time snapshot of technical configuration. It does not replace "
    "legal review of Data Processing Agreements, a formal SOC 2 Type II engagement, or formal "
    "counsel on FERPA or state-law compliance questions. FERPA is enforced by the U.S. Dept. "
    "of Education Student Privacy Policy Office (SPPO) on a complaint-driven basis; state "
    "student-privacy laws are enforced by state attorneys general and state departments of "
    "education on their own schedules."
)
...
<div class="footer">Generated from <code>{html.escape(md_path.name)}</code> by the FERPA compliance skill.
